...
class TileTrainingData:
    HEADERS = ['remaining_mpls', 'remaining_abbots', 'feature_finished', 'terrain_type', 'reigning_player', 'is_end_farmer_time', 'feature_mpl_points',
               'feature_possible_points', 'feature_point_difference', 'feature_less_open_edges', 'feature_mpl_on_street',
               'selected']
    ...
    def __init__(self, training_data: dict):
        self.data = {'remaining_mpl': int(training_data['remaining_mpl']),
                     ...
                     'reigning_player': int(training_data['reigning_player']),
                     'is_end_farmer_time': int(training_data['is_end_farmer_time']),
                     'feature_mpl_points': int(training_data['feature_mpl_points']),
                     'feature_possible_points': int(training_data['feature_possible_points']),
                     'feature_point_difference': int(training_data['feature_point_difference']),
                     'feature_less_open_edges': int(training_data['feature_less_open_edges']),
    ...
    def get_list(self) -> list:
        return [self.data['remaining_mpl'],
                ...
                self.data['reigning_player'],
                self.data['is_end_farmer_time'],
                self.data['feature_mpl_points'],
                self.data['feature_possible_points'],
                self.data['feature_point_difference'],
                self.data['feature_less_open_edges'],
