...
class MyPlayerDecisionTree(Player):
    def __init__(self, name, debug=False, core_number=1):
        # Player initialization
        self.debug = debug
        # Tile Tree loading
        #self.loaded_model_tile_entropy = jb.load(p.TILE_ENTROPY_TREE_PATH)
        self.loaded_model_tile_gini = jb.load(p.TILE_GINI_TREE_PATH)

        # mpl Tree loading
        #self.loaded_model_mpl_entropy = jb.load(p.mpl_ENTROPY_TREE_PATH)
        self.loaded_model_mpl_gini = jb.load(p.MPL_GINI_TREE_PATH)

        super().__init__(uuid.uuid4(), name, PlayerType.AI)

    def make_mpl_prediction(self, move_data: list, mode="gini"):
        if mode == "entropy":
            loaded_model: DecisionTreeClassifier = self.loaded_model_mpl_entropy
        elif mode == "gini":
            loaded_model: DecisionTreeClassifier = self.loaded_model_mpl_gini
        else:
    ...
    def make_tile_prediction(self, move_data: list, mode="gini"):
        if mode == "entropy":
            loaded_model: DecisionTreeClassifier = self.loaded_model_tile_entropy
        elif mode == "gini":
            loaded_model: DecisionTreeClassifier = self.loaded_model_tile_gini
        else:
            print("No Valid Mode or Model, Closing Program")
            return
